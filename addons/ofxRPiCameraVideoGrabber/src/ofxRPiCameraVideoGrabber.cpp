...
void ofxRPiCameraVideoGrabber::setup(OMXCameraSettings omxCameraSettings)
{
	...
	//pix.allocate(1280,720,OF_IMAGE_COLOR_ALPHA);
...
void ofxRPiCameraVideoGrabber::setMirrorType(OMX_MIRRORTYPE mirror_ ) // OMX_Mirror{None,Vertical,Horizontal,Both}
{	
	mirror = mirror_;
	...
	mirrorConfig.nPortIndex = 71;//OMX_ALL;
	mirrorConfig.eMirror = mirror; 
	
	error = OMX_SetConfig(camera, OMX_IndexConfigCommonMirror, &mirrorConfig);
	if(error != OMX_ErrorNone) 
	{
		ofLog(OF_LOG_ERROR, "camera setMirror FAIL error: 0x%08x", error);
	}
}
